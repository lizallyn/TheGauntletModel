...
for(t in 1:(days - 1)) {
  ...
  if(num_seals_at_gauntlet == 0 | consumed_by_pv == 0) {
    ...
    if(num_seals_at_gauntlet > consumed_by_pv){
      seals_that_eat <- sample(seals_at_gauntlet, consumed_by_pv)
      ...
      seals_that_eat_extra <- sample(seals_at_gauntlet, remainder)
  ...
  if(t %in% harvest_days_all && fear == "Fear"){
    ...
    if(num_seals_exposed_fear > 0){
      seals_exposed_fear <- sample(size = num_seals_exposed_fear, 
                                   x = seals_at_gauntlet)
