...
ballard_salmon_kcal <- salmon_kcal[,c("Sockeye", "ChinookB", "CohoB")]
...
eaten_salmon_pv <- empty_salmon_df
eaten_salmon_zc <- empty_salmon_df
...
if(num_seals > 0) {
  ...
  calories_consumed_pv <- twoDzeroes
  ...
  if(scenario == "Trap") {
    scenario_pv <- "None"
  } else {scenario_pv <- scenario}
  harvest_plan_pv <- createHarvestPlan(scenario = scenario_pv,
                                       timing = timing,
...
if(num_zc > 0) {
  ...
  calories_consumed_zc <- twoDzeroes_zc
