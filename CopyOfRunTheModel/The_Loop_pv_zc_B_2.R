...
for(t in 1:(days - 1)) {
  ...
  if(t %in% sealion_days) {
    ...
    for(csl in 1:num_zc){
      ...
      if(is.na(social_info)){
        P_social_zc[csl,t] <- zc_prob_gauntlet[csl,t]
      } else {
  ...
  if(t %in% hot_days && temps == "Hot!"){
    escape_rates <- run_info$Escape / temp_slow_factor
  } else {escape_rates <- run_info$Escape}
  ...
  eaten_salmon[t, 2:ncol(escape_salmon)] <- salmon_result[, "C"] +
    salmon_result[, "C_SL"]
  eaten_salmon_pv[t, 2:ncol(escape_salmon)] <- salmon_result[,"C"]
  eaten_salmon_zc[t, 2:ncol(escape_salmon)] <- salmon_result[,"C_SL"]
  consumed_total[t] <- sum(eaten_salmon[t, 2:ncol(escape_salmon)])
  
  # assign consumed salmon calories to gauntlet pinnipeds
  
  consumed_by_pv <- sum(salmon_result[,"C"])
  consumed_by_zc <- sum(salmon_result[,"C_SL"])
  
  pv_calorie_vector <- unname(unlist(rep(ballard_salmon_kcal, salmon_result[,"C"])))
  zc_calorie_vector <- unname(unlist(rep(ballard_salmon_kcal, salmon_result[,"C_SL"])))
  
  if(length(pv_calorie_vector) > 1){
    pv_calorie_vector <- sample(pv_calorie_vector)
  }
  
  if(length(zc_calorie_vector) > 1){
    zc_calorie_vector <- sample(zc_calorie_vector)
  }
  
  #pv
  if(num_seals_at_gauntlet == 0 | consumed_by_pv == 0) {
    calories_consumed_pv[,t] <- 0
  } else {
    if(num_seals_at_gauntlet >= consumed_by_pv){
      seals_that_eat <- sampleBespoke(seals_at_gauntlet, consumed_by_pv)
      calories_consumed_pv[seals_that_eat, t] <- pv_calorie_vector
    } else {
      each_gets_whole <- floor(consumed_by_pv/num_seals_at_gauntlet)
      for(seal in 1:num_seals_at_gauntlet){
        calories_consumed_pv[seals_at_gauntlet[seal],t] <- sum(pv_calorie_vector[1:each_gets_whole])
        pv_calorie_vector <- pv_calorie_vector[-(1:each_gets_whole)]
      }
      leftovers <- consumed_by_pv %% num_seals_at_gauntlet
      seals_that_eat_extra <- sampleBespoke(seals_at_gauntlet, leftovers)
      calories_consumed_pv[seals_that_eat_extra, t] <- calories_consumed_pv[seals_that_eat_extra, t] + pv_calorie_vector
    }
  }
  ...
  if(num_zc_at_gauntlet == 0 | consumed_by_zc == 0){
    ...
    if(num_zc_at_gauntlet >= consumed_by_zc){
      ...
      salmon_consumed_zc[zc_that_eat,t] <- zc_calorie_vector
    } else {
      each_gets_whole <- floor(consumed_by_zc/num_zc_at_gauntlet)
      for(csl in 1:num_zc_at_gauntlet){
        calories_consumed_zc[zc_at_gauntlet[csl],t] <- sum(zc_calorie_vector[1:each_gets_whole])
        zc_calorie_vector <- zc_calorie_vector[-(1:each_gets_whole)]
      }
      leftovers <- consumed_by_zc %% num_zc_at_gauntlet
      zc_that_eat_extra <- sampleBespoke(zc_at_gauntlet, leftovers)
      calories_consumed_zc[zc_that_eat_extra, t] <- calories_consumed_zc[zc_that_eat_extra, t] + zc_calorie_vector
    }
  }
  ...
  # assign and track harvested pinnipeds
  if(H[t] > 0){
    killed_pv <- sampleBespoke(seals_at_gauntlet, H[t])
    kill_list <- c(kill_list, killed_pv)
  }
  if(H_zc[t] > 0){
    killed_zc <- sampleBespoke(x = c(as.numeric(zc_at_gauntlet)), size = H_zc[t])
    kill_list_zc <- c(kill_list_zc, killed_zc)
  }
  ...
  for(seal in 1:num_seals){
    ...
    if(seal %in% kill_list){
      ...
      update_output <- updateLearning(salmon_consumed = calories_consumed_pv[seal, t], 
                                      boats = boat_days[t], 
                                      rho = rho, 
  ...
  for(csl in 1:num_zc){
    ...
    if(csl %in% c(unlist(kill_list_zc), gone_forever)){
      ...
      update_output <- updateLearning(salmon_consumed = calories_consumed_zc[csl, t], 
                                      w = w["Zc"], 
                                      hunting = hunt_zc[csl], 
                                      boats = boat_days[t],
                                      x_t = x_zc[csl, t], 
  ...
  if((t+1) == sealion_arrival){
    if(length(kill_list_zc) == 0){
      ...
      zc_alive <- zc_list[-unlist(kill_list_zc)]
      fall_zc_alive <- fall_zc[which(fall_zc %in% zc_alive)]
      gone_forever <- zc_list[-fall_zc_alive]
      zc_prob_gauntlet[gone_forever,t+1] <- NA
      zc_forage_loc[gone_forever, t+1] <- NA
      x_zc[gone_forever, t+1] <- NA
      y_zc[gone_forever, t+1] <- NA
