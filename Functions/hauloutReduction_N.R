...
if(reduction > 0){
  ...
  zc_reduction <- hauloutReduction(reduction, network_zc, num_zc)
  num_zc <- zc_reduction$Num
  network_zc <- zc_reduction$Mat
  rownames(network_zc) <- 1:num_zc
  colnames(network_zc) <- 1:num_zc
  
  ej_reduction <- hauloutReduction(reduction, network_ej, num_ej)
  num_ej <- ej_reduction$Num
  network_ej <- ej_reduction$Mat
